...
if ( ! function_exists( 'obsius_core_add_portfolio_list_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function obsius_core_add_portfolio_list_shortcode( $shortcodes ) {
		$shortcodes[] = 'ObsiusCore_Portfolio_List_Shortcode';

		return $shortcodes;
	}

	add_filter( 'obsius_core_filter_register_shortcodes', 'obsius_core_add_portfolio_list_shortcode' );
}

if ( class_exists( 'ObsiusCore_List_Shortcode' ) ) {
	class ObsiusCore_Portfolio_List_Shortcode extends ObsiusCore_List_Shortcode {
		...
		public function __construct() {
			...
			$this->set_layouts( apply_filters( 'obsius_core_filter_portfolio_list_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'obsius_core_filter_portfolio_list_extra_options', array() ) );
			$this->set_hover_animation_options( apply_filters( 'obsius_core_filter_portfolio_list_hover_animation_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( OBSIUS_CORE_CPT_URL_PATH . '/portfolio/shortcodes/portfolio-list' );
			$this->set_base( 'obsius_core_portfolio_list' );
			$this->set_name( esc_html__( 'Portfolio List', 'obsius-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays list of portfolios', 'obsius-core' ) );
			$this->set_scripts( apply_filters( 'obsius_core_filter_portfolio_list_register_assets', array() ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'select',
					'name'          => 'disable_view_cursor',
					'title'         => esc_html__( 'Disable View Cursor', 'obsius-core' ),
					'options'       => obsius_core_get_select_type_options_pool( 'no_yes', false ),
					'default_value' => 'no',
					'dependency'    => array(
						'show' => array(
							'layout' => array(
								'values'        => array( 'info-below' ),
            ...
            $this->set_option(
                array(
                    'field_type'    => 'select',
                    'name'          => 'pinterest_slider',
                    'title'         => esc_html__( 'Pinterest Slider', 'obsius-core' ),
                    'description'   => esc_html__( 'When enabled, Number of Columns and Columns Responsive options won\'t take effect.', 'obsius-core' ),
                    'options'       => obsius_core_get_select_type_options_pool( 'no_yes', false ),
                    'default_value' => 'no',
                    'dependency'    => array(
                        'show' => array(
            ...
            $this->set_option(
                array(
                    'field_type'    => 'select',
                    'name'          => 'pinterest_slider_layout',
                    'title'         => esc_html__( 'Pinterest Layout', 'obsius-core' ),
                    'options'       => array(
                        'pinterest-one' => esc_html__( 'Pinterest One', 'obsius-core' ),
                        'pinterest-two' => esc_html__( 'Pinterest Two', 'obsius-core' ),
                    ),
                    'default_value' => 'pinterest-one',
                    'dependency'    => array(
                        'show' => array(
                            'pinterest_slider' => array(
                                'values'        => 'yes',
                                'default_value' => 'no',
                            ),
                        ),
            ...
            $this->set_option(
                array(
                    'field_type'    => 'hidden',
                    'name'          => 'exclude_decoration',
                    'title'         => esc_html__( 'Exclude Items With Decoration', 'obsius-core' ),
                    'options'       => obsius_core_get_select_type_options_pool( 'no_yes', false ),
                    'default_value' => 'no',
                )
            );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'obsius_core_portfolio_list', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function load_assets() {
			...
			do_action( 'obsius_core_action_portfolio_list_load_assets', $this->get_atts() );
		}
		...
		public function render( $options, $content = null ) {
			...
			$atts['data_attr']         = obsius_core_get_pagination_data( OBSIUS_CORE_REL_PATH, 'post-types/portfolio/shortcodes', 'portfolio-list', 'portfolio', $atts );

			$atts['this_shortcode'] = $this;

			return obsius_core_get_template_part( 'post-types/portfolio/shortcodes/portfolio-list', 'templates/content', $atts['behavior'], $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'yes' === $atts['disable_view_cursor'] ? 'qodef-view-cursor--disabled' : '';
		...
		public function get_item_classes( $atts ) {
            ...
            if ( ! empty( $replace_image_with_decoration ) ) {
                $list_item_classes[] = 'qodef-with--' . $replace_image_with_decoration;
            }

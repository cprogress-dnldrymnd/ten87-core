	...
	<div class="qodef-left-info">
		<div class="qodef-left-info-content">
			<?php if ( ! empty( $info_title ) ) { ?>
				<h2 class="qodef-e-title entry-title"><?php echo qode_framework_wp_kses_html( 'content', $info_title ); ?></h2>
			<?php } ?>

			<?php if ( ! empty( $info_text ) ) { ?>
				<p class="qodef-e-text"><?php echo esc_html( $info_text ); ?></p>
			<?php } ?>
			...
			if ( ! empty( $button_params ) && ! empty( $button_params['text'] ) && class_exists( 'ObsiusCore_Button_Shortcode' ) ) {

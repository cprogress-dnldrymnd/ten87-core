...
if ( ! function_exists( 'obsius_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function obsius_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'ObsiusCore_Image_Gallery_Shortcode';

		return $shortcodes;
	}

	add_filter( 'obsius_core_filter_register_shortcodes', 'obsius_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'ObsiusCore_List_Shortcode' ) ) {
	class ObsiusCore_Image_Gallery_Shortcode extends ObsiusCore_List_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( OBSIUS_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'obsius_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'obsius-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'obsius-core' ) );
			$this->set_scripts(
				array(
			...
			$this->set_option(
				array(
					...
					'name'       => 'images',
					'multiple'   => 'yes',
					'title'      => esc_html__( 'Images', 'obsius-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->map_list_options(
				array(
					...
					'group'            => esc_html__( 'Gallery Settings', 'obsius-core' ),
				)
			);
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'obsius_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return obsius_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}

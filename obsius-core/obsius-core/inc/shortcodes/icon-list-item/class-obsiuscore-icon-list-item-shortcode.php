...
if ( ! function_exists( 'obsius_core_add_icon_list_item_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function obsius_core_add_icon_list_item_shortcode( $shortcodes ) {
		$shortcodes[] = 'ObsiusCore_Icon_List_Item_Shortcode';

		return $shortcodes;
	}

	add_filter( 'obsius_core_filter_register_shortcodes', 'obsius_core_add_icon_list_item_shortcode' );
}

if ( class_exists( 'ObsiusCore_Shortcode' ) ) {
	class ObsiusCore_Icon_List_Item_Shortcode extends ObsiusCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( OBSIUS_CORE_SHORTCODES_URL_PATH . '/icon-list-item' );
			$this->set_base( 'obsius_core_icon_list_item' );
			$this->set_name( esc_html__( 'Icon List Item', 'obsius-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon list item element', 'obsius-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'content_alignment_pad',
					'title'      => esc_html__( 'Content Alignment Bellow 1024', 'obsius-core' ),
					'options'    => array(
						''       => esc_html__( 'Default', 'obsius-core' ),
						'left'   => esc_html__( 'Left', 'obsius-core' ),
						'center' => esc_html__( 'Center', 'obsius-core' ),
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'content_alignment_mobile',
					'title'      => esc_html__( 'Content Alignment on Mobile Devices', 'obsius-core' ),
					'options'    => array(
						''       => esc_html__( 'Default', 'obsius-core' ),
						'left'   => esc_html__( 'Left', 'obsius-core' ),
						'center' => esc_html__( 'Center', 'obsius-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'obsius_core_icon_list_item', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return obsius_core_get_template_part( 'shortcodes/icon-list-item', 'templates/icon-list-item', '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['content_alignment_pad'] ) ? 'qodef-alignment-pad--' . $atts['content_alignment_pad'] : 'qodef-alignment-pad--left';
			$holder_classes[] = ! empty( $atts['content_alignment_mobile'] ) ? 'qodef-alignment-mobile--' . $atts['content_alignment_mobile'] : 'qodef-alignment-mobile--left';

...
class ObsiusCore_Top_Area {
	...
	public function __construct() {
		...
		add_action( 'obsius_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'obsius_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
		add_filter( 'obsius_core_filter_top_area_inner_class', array( $this, 'set_grid_class' ) );
	}

	/**
	 * @return ObsiusCore_Top_Area
	 */
	...
	function is_top_area_enabled() {
		$top_area_enabled = 'yes' === obsius_core_get_post_value_through_levels( 'qodef_top_area_header' );
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
	...
	public function is_top_area_transparent() {
		$background_color = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		...
		obsius_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$height       = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$side_padding = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
		$border_color = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_border_color' );
		$border_width = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_border_width' );
		$border_style = obsius_core_get_post_value_through_levels( 'qodef_top_area_header_border_style' );
	...
	public function set_grid_class( $classes ) {
		if ( 'yes' === obsius_core_get_post_value_through_levels( 'qodef_top_area_header_in_grid' ) ) {
...
ObsiusCore_Top_Area::get_instance();

	...
	var elementorSection = {
		init          : function ($scope) {
			...
			if (isEditMode && typeof $scope !== 'undefined') {
				...
				// gradient in background options
				if (typeof sectionData.qodef_enable_gradient_in_background !== 'undefined') {
					settings['enable_gradient'] = sectionData.qodef_enable_gradient_in_background;
				}

				if (typeof sectionData.qodef_offset_vertical_position !== 'undefined') {
					settings['gradient_vertical_position'] = sectionData.qodef_gradient_vertical_position;
				}

				if (typeof sectionData.qodef_offset_horizontal_position !== 'undefined') {
					settings['gradient_horizontal_position'] = sectionData.qodef_gradient_horizontal_position;
				}
					...
					function (index, properties) {
						...
						properties.forEach(
							function (property) {
								...
								if (typeof property['enable_gradient_in_background'] !== 'undefined' && property['enable_gradient_in_background'] === 'gradient') {

									$target                                  = $( '[data-id="' + index + '"]' );
									settings['enable_gradient_in_background']              = property['enable_gradient_in_background'];
									settings['gradient_vertical_position']   = property['gradient_vertical_position'];
									settings['gradient_horizontal_position'] = property['gradient_horizontal_position'];

									if (typeof settings['enable_gradient_in_background'] !== 'undefined') {
										settings['enable_gradient'] = 'gradient';
									}
								}
		...
		generateOutput: function ($target, settings) {
			...
			if (typeof settings['enable_gradient'] !== 'undefined' && settings['enable_gradient'] === 'gradient') {

				$( '.qodef-gradient-in-background-holder', $target ).remove();
				$target.removeClass( 'qodef-gradient-in-background' );

				var $layout = null;

				$target.addClass( 'qodef-gradient-in-background' );

				$layout = $( '<div class="qodef-gradient-in-background-holder" style="position: absolute; z-index: -1; ' + 'top:' + settings['gradient_vertical_position'] + ';' + 'left:' + settings['gradient_horizontal_position'] + '"></div>' ).prependTo( $target );

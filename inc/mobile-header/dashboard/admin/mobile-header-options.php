...
if ( ! function_exists( 'obsius_core_add_mobile_header_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function obsius_core_add_mobile_header_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'slug'        => 'mobile-header',
				'icon'        => 'fa fa-cog',
				'title'       => esc_html__( 'Mobile Header', 'obsius-core' ),
				'description' => esc_html__( 'Global Mobile Header Options', 'obsius-core' ),
			)
		);
		...
		if ( $page ) {
			...
			$general_tab->add_field_element(
				array(
					...
					'title'         => esc_html__( 'Sticky Mobile Header', 'obsius-core' ),
					'description'   => esc_html__( 'Set mobile header to be sticky', 'obsius-core' ),
				)
			);
			...
			$general_tab->add_field_element(
				array(
					...
					'default_value' => apply_filters( 'obsius_core_filter_mobile_header_layout_default_option', '' ),
					'options'       => apply_filters( 'obsius_core_filter_mobile_header_layout_option', array() ),
				)
			);
			...
			$section_svg_path->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Mobile Menu Open Icon SVG Path', 'obsius-core' ),
					'description' => esc_html__( 'Enter your mobile menu open icon SVG path here. Please remove version and id attributes from your SVG path because of HTML validation', 'obsius-core' ),
				)
			);
			...
			$section_svg_path->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Mobile Menu Close Icon SVG Path', 'obsius-core' ),
					'description' => esc_html__( 'Enter your mobile menu close icon SVG path here. Please remove version and id attributes from your SVG path because of HTML validation', 'obsius-core' ),
				)
			);
			...
			$opener_section = $general_tab->add_section_element(
				array(
					...
					'title' => esc_html__( 'Mobile Header Opener Styles', 'obsius-core' ),
				)
			);
			...
			$opener_section_row->add_field_element(
				array(
					...
					'title'      => esc_html__( 'Hover/Active Color', 'obsius-core' ),
					'args'       => array(
						'col_width' => 3,
					),
				)
			...
			do_action( 'obsius_core_action_after_mobile_header_options_map', $page, $general_tab );
		}
	}

	add_action( 'obsius_core_action_default_options_init', 'obsius_core_add_mobile_header_options', obsius_core_get_admin_options_map_position( 'mobile-header' ) );
}

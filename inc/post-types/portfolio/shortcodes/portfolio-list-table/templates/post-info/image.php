<?php
$portfolio_list_table = get_post_meta( get_the_ID(), 'qodef_portfolio_list_image', true );
$has_image                         = ! empty( $portfolio_list_table ) || has_post_thumbnail();
$image_id                          = ! empty( $portfolio_list_table ) ? $portfolio_list_table : get_post_thumbnail_id();

if ( $has_image ) {
	$portfolio_url       = obsius_core_get_portfolio_list_table_item_url( get_the_ID() );
	...
	<div class="qodef-e-media-image">
		<a itemprop="url" href="<?php echo esc_url( $portfolio_url['link'] ); ?>" target="<?php echo esc_attr( $portfolio_url['target'] ); ?>">
			<?php echo obsius_core_get_list_shortcode_item_image( $image_dimension, $portfolio_list_table, $custom_image_width, $custom_image_height ); ?>
		</a>

<?php
$portfolio_list_double_image = get_post_meta( get_the_ID(), 'qodef_portfolio_list_image', true );
$has_image                         = ! empty( $portfolio_list_double_image ) || has_post_thumbnail();
$image_id                          = ! empty( $portfolio_list_double_image ) ? $portfolio_list_double_image : get_post_thumbnail_id();
...
if ( $has_image ) {
	...
	<div class="qodef-e-media-image">
		<a itemprop="url" href="<?php echo esc_url( $portfolio_url['link'] ); ?>" target="<?php echo esc_attr( $portfolio_url['target'] ); ?>">
			<?php echo obsius_core_get_list_shortcode_item_image( $image_dimension, $portfolio_list_double_image, $custom_image_width, $custom_image_height ); ?>
		</a>

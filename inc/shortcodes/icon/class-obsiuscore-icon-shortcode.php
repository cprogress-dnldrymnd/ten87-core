...
if ( ! function_exists( 'obsius_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function obsius_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'ObsiusCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'obsius_core_filter_register_shortcodes', 'obsius_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'ObsiusCore_Shortcode' ) ) {
	class ObsiusCore_Icon_Shortcode extends ObsiusCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( OBSIUS_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'obsius_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'obsius-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'obsius-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'obsius-core' ),
						'small'   => esc_html__( 'Small', 'obsius-core' ),
						'medium'  => esc_html__( 'Medium', 'obsius-core' ),
						'large'   => esc_html__( 'Large', 'obsius-core' ),
						'huge'    => esc_html__( 'Huge', 'obsius-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'obsius-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'obsius-core' ),
					'group'      => esc_html__( 'Style', 'obsius-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'obsius-core' ),
					'group'      => esc_html__( 'Style', 'obsius-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'obsius_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return obsius_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
